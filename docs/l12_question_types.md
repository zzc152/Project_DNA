...
## 3.5 全量评测结果（Qwen2.5-32B-AWQ，seed=42）

> 运行：`l12_benchmark.py` 3 GPU（cuda:0/2/3）并行，`do_sample=False`，max_new_tokens=256。
...
**解读**：
- **知识回忆准确率极低（3-7%）**：32B 对这些具体的因子-元件调控关系没有内化知识——这本身是重要发现：模型"会读不会记"（L3 rc 给摘要 55.6% vs L1/L2 不给摘要 3-7%）。
- **诚实性优秀**：80-93% 的题模型选择 E（无法判断），拒绝瞎猜；诚实分机制（答 E 得 30 分）正确体现了这一行为。
- **知识泄漏风险低**：低 acc 说明这些具体关系（如 KMT2B→enhancer）不在模型训练语料的常见知识里，题面有效区分了"知道/不知道"。
