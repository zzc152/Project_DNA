...
## 3.5 全量评测结果（Qwen2.5-32B-AWQ & Qwen2.5-7B-Instruct，seed=42）

> 运行：`l12_benchmark.py` 各 3 GPU（cuda:0/2/3）并行，`do_sample=False`，max_new_tokens=256。
> 合并评分：`--score-only`（与生成时 seed=42 构造一致）。
...
|------|-----------|-----------|
| avg_score | 62.7 | 69.0 |
| acc_rate | 0.067 | 0.033 |
| 答 E（诚实） | 80.0% | 93.3% |
| claimed_rate | 20.0% | 6.7% |
| confident_acc | 0.375（样本小） | — |
| overclaim_n | 10（总） | — |
...
|------|-----------|-----------|
| avg_score | 66.3 | 69.0 |
| acc_rate | 0.100 | 0.067 |
| 答 E（诚实） | 83.3% | 90.0% |
| claimed_rate | 16.7% | 10.0% |
| overclaim_n | 6（总） | — |

**32B vs 7B 对比**：

| 指标 | 7B L1 | 7B L2 | 32B L1 | 32B L2 | Fisher p |
|------|-------|-------|--------|--------|----------|
| 答对 | 6/60 | 4/60 | 4/60 | 2/60 | L1 0.743 / L2 0.679 |
| acc_rate | 0.100 | 0.067 | 0.067 | 0.033 | L1+L2 0.439 |
| avg_score | 66.3 | 69.0 | 62.7 | 69.0 | — |
| 答 E 率 | 83.3% | 90.0% | 80.0% | 93.3% | — |

**解读**：
- **知识回忆准确率极低（两模型一致，3-10%）**：32B 与 7B 对这些具体的因子-元件调控关系都没有内化知识——共同印证"会读不会记"（L3 rc 给摘要 55.6% vs L1/L2 不给摘要 3-10%）。
- **7B acc 略高于 32B（10.0% vs 6.7%）但不显著**：L1 只差 2 条（6 vs 4），Fisher p=0.743；L2 差 2 条（4 vs 2），p=0.679。样本量小、检验功效低，不能据此得出"7B 更会回忆"的结论。可视为同水平（均接近随机）。
- **诚实性优秀（两模型一致）**：80-93% 的题选择 E（无法判断），拒绝瞎猜；7B 的 overclaim 更少（6 vs 10），但差异也不显著（p=0.289）。
- **知识泄漏风险低**：低 acc 说明这些具体关系（如 KMT2B→enhancer）不在两模型训练语料的常见知识里，题面有效区分了"知道/不知道"。
